$(function(){
	...
	bind_ajax_remove(site_url('user','removeuser'),function(userid){
		...
		var msg = L('user.remove_admin_tips');
	...
	$('.edit_user').click(function(){
		...
		var bind_edit = function(){
			$.get(site_url('user','userinfo','userid='+userid),function(json){
				...
				$(':radio[name="priv"][value="'+info.priv+'"]').attr('checked',true);
				// 注册提交
				$('#submitbtn').unbind().click(function(){
					// 测试输入
					if(! checkinput(false)){
	...
	$('#searchbut').click(function(){
		...
		location.href = site_url('user','adminuser',params);
	});
	
	// 显示所有
	$('#showall').click(function(){
		location.href = site_url('user','adminuser');
		return false;
	});
